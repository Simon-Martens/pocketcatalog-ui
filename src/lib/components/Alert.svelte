...
<div class="relative mt-3 mb-3 pl-1" on:mouseenter={() => (mouseover = true)} on:mouseleave={() => (mouseover = false)}>
    <div
      ...
      {#if alert.type === "info"}
        ...
        >
          ...
          {#if alert.ok_button && alert.callback}
            ...
            >
              <span class="underline decoration-dotted hover:decoration-solid px-1 p-0.5 hover:bg-sky-300 rounded hover:text-sky-900 ml-1"
  ...
  {:else if (mouseover && !debounceclose) || !$loggedIn}
    <div class="alert bg-slate-100" in:fly={{x: 30, duration: 450}} out:fly={{x: 30, duration: 450}}>
      ...
      {:else if $loggedIn}
        Server verbunden; keine Meldungen. <a class="underline decoration-dotted hover:decoration-solid px-1 p-0.5 text-slate-700 hover:text-slate-900 ml-1" href="/login" data-sveltekit-preload-data="off">Abmelden</a>
      {:else}
